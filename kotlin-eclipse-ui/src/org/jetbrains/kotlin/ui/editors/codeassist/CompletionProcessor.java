...
public class CompletionProcessor implements IContentAssistProcessor, ICompletionListener {
    ...
    @NotNull
    private Collection<ICompletionProposal> generateSimpleCompletionProposals(@NotNull ITextViewer viewer, int offset, int offset2, @NotNull String identifierPart) {
        ...
        BindingContext context = analyzeProjectInForeground(javaProject);
        
        if (context == null) {
            return Collections.emptyList();
        }
        
        Collection<DeclarationDescriptor> declarationDescriptors = getReferenceVariants(expression, context);
        ...
        for (DeclarationDescriptor descriptor : declarationDescriptors) {
            ...
            if (completion.contains(psiElement.getText())) {
                proposals.add(new CompletionProposal(completion, offset, offset2 - offset, completion.length()));
            }
        }
    ...
    private Collection<DeclarationDescriptor> excludePrivateDescriptors(@NotNull Collection<DeclarationDescriptor> descriptors) {
    ...
    private int getIdentifierStartOffset(String text, int offset) {
        ...
        while ((identStartOffset != 0) && 
                (text.charAt(identStartOffset - 1) == '.' || Character.isUnicodeIdentifierPart(text.charAt(identStartOffset - 1)))) {
            identStartOffset--;
        }
